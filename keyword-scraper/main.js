    ...
    return {
        ...
        StartWork: function() {
            if (this.doWork === false) {
                // reset these
                this.saveSettings();
                $('#startjob').val('Stop Job').text('Stop Scraper').addClass('btn-danger');
                this.hashMapInputs = {};
                ...
                if (!this.keywordsToQuery.length)
                    ...
                    var untickedInputs = this.keywordsToQuery.filter(function(k){
                        return k.slice(-1)!=='âœ“' && k.slice(-1)!=='âŒ';
                    });
        ...
        DoJob: function() {
            if (this.doWork === true && this.queryLock === false) {
                if (this.keywordsToQueryIndex < this.numOfInitialKeywords) {
                    ...
                    if (currentKw.slice(-1)!=='âœ“' && currentKw.slice(-1)!=='âŒ') {
        ...
        markAsDone: function(search){
            ...
            if (this.keywordsToQuery[this.keywordsToQueryIndex]===search)
                this.keywordsToQuery[this.keywordsToQueryIndex]+=' âœ“';
            else if (this.keywordsToQuery[this.keywordsToQueryIndex-1]===search)
                this.keywordsToQuery[this.keywordsToQueryIndex-1]+=' âœ“';
            else
        ...
        markAsNone: function(search){
            // mark as done in queue
            if (this.keywordsToQuery[this.keywordsToQueryIndex]===search)
                this.keywordsToQuery[this.keywordsToQueryIndex]+=' âŒ';
            else if (this.keywordsToQuery[this.keywordsToQueryIndex-1]===search)
                this.keywordsToQuery[this.keywordsToQueryIndex-1]+=' âŒ';
            else
                console.warn('Cant find ',search,'in keywordsToQuery');
        },
